...
async def run_rijeka_exit_simulation():
    ...
    while True:
        ...
        for e in entrances:
            ...
            if origin == "PULA-ENTRANCE":
                if (vehicle_id in cam1_passed) and (vehicle_id not in cam2_passed):
                    must_exit = True
                    travel_time = TRAVEL_TIME_FROM_PULA

            elif origin == "UMAG-ENTRANCE":
                if (vehicle_id in cam1_passed) and (vehicle_id in cam2_passed):
                    must_exit = True
            ...
            print(f"[RIJEKA-EXIT] Vozilo {vehicle_id} ({origin}) izašlo u {reading.timestamp}")
