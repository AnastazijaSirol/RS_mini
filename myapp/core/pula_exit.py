...
async def run_pula_exit_simulation():
    ...
    while True:
        ...
        for e in entrances:
            ...
            if origin == "RIJEKA-ENTRANCE":
                if (vehicle_id not in cam2_passed) and (vehicle_id in cam1_passed):
                    must_exit = True
                    travel_time = TRAVEL_TIME_FROM_RIJEKA

            elif origin == "UMAG-ENTRANCE":
                if (vehicle_id not in cam1_passed) and (vehicle_id in cam2_passed):
                    must_exit = True
            ...
            print(f"[PULA-EXIT] Vozilo {vehicle_id} ({origin}) izlazi u {reading.timestamp}")
